...
for year in [2021,2022]:
    df_fc = data_voorspelling[data_voorspelling['Jaar'].dt.year==year].copy()
    ...
    for iso in landen:
        ...
        i = sub.index[0]; ci = sub.at[i,'land_idx']
        lv = last_vals[last_vals['ISO3']==iso].sort_values('Jaar')
        y1 = lv['kwetsbaarheid'].iloc[-1]; x1 = {var: lv[var].iloc[-1] for var in variabelen_kort}; x2 = {var: lv[var].iloc[-2] for var in variabelen_kort}
        mu_fc = phi_y1_mean[ci]*y1 + sum(phi_x1_mean[var][ci]*x1[var] + phi_x2_mean[var][ci]*x2[var] for var in variabelen_kort)
        df_fc.at[i,'kwetsbaarheid_forecast'] = mu_fc
        new = {'ISO3':iso,'Jaar':pd.to_datetime(f"{year}-12-31"),'kwetsbaarheid':mu_fc}
        for var in variabelen_kort: new[var] = sub.at[i,var]
        records.append(new)
...
\\midrule
Bayesiaanse ARDL(1,2) met shrinkage & {aic:.2f} & {bic:.2f} & {mae_out:.4f} & {rmse_out:.4f} \\\
\\bottomrule
\\end{{tabular}}
\\caption{{ARDL(1,2) met Bayesian shrinkage: In-sample (AIC, BIC) en out-of-sample forecast errors (2021--2022)}}
\\label{{tab:ardl12_bayes_shrinkage}}
\\end{{table}}
"""
print(latex)
